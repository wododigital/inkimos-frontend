...
import icon from '../../assets/IMG 5.png';
...
const Header = () => {
  ...
  const navigation = [
    ...
    { name: 'Insights', href: 'https://inkimos.com/insights/' },
  ...
  const whatWeDoLinks = servicesData.flatMap((category) => [
    category.viewAllLink,
    ...category.services.map((service) => service.link),
  ]);
  const isActive = whatWeDoLinks.includes(location.pathname);
      ...
      <nav aria-label="Global" className="max-w-[1440px] mx-auto flex items-center justify-between lg:p-2.5 px-5 lg:px-12 relative">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          <div className='menu-item group'>
            <a to='#' className={`font-semibold hover:text-green flex items-center gap-2 ${isActive ? 'text-green' : ''}`}>
              What we do <FaCaretDown className="transition-transform duration-300 group-hover:rotate-180 " />
            </a>
          ...
          <div className='menu-item group'>
            <a to='#' className={`font-semibold hover:text-green flex items-center gap-2  ${location.pathname === '/industry-expertise' ? 'text-green' : ''}`}>
              Industry Expertise <FaCaretDown className="transition-transform duration-300 group-hover:rotate-180 " />

            </a>
            <div className="megamenu text-black p-5 px-10">
              <div className='border-b-[1px] border-green'>
                <h2 className='text-2xl'>Industries</h2>
              </div>
              <div className="flex flex-wrap justify-between">
              ...
              <div className='my-3 '>
                <Link to='/industry-expertise' className='border-b-[1px] border-black text-xl font-semibold  ms-3 hover:ms-8 transition-all duration-500 flex items-center gap-4 w-44'> <span>Discover More </span><img src={icon} className='w-5 h-1 underline' alt='small-icon' /></Link>
              </div>
            </div>
          </div>

          <Link to='/about' className={`font-semibold hover:text-green ${location.pathname === '/about' ? 'text-green' : ''
            }`}>
            Know Inkimos
          </Link>
          <a href='https://inkimos.com/insights/' className='font-semibold hover:text-green'>
            Insights
          </a>
          <Link to='/careers' className={`font-semibold hover:text-green ${location.pathname === '/careers' ? 'text-green' : ''
            }`}>
            Careers
          </Link>
        </div>
              ...
              <div className="space-y-4 py-6 ">
                <div className="-mx-3">
                  ...
                  >
                    What we do
                    <svg
                      className={`h-5 w-5 flex-none transition-all ${navMenuOpen.services ? "rotate-90" : ""
                        }`}
                      viewBox="0 0 20 20"
                      fill="#fff"
                      aria-hidden="true"
                ...
                <a
                  href="https://inkimos.com/insights/"
                  className="-mx-3 block px-3 py-2 text-base leading-7 text-white hover:text-green"
                >
                  Insights
                </a>
